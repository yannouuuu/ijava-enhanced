...
public class StatusCommand implements Command {
    ...
    private void showSpecificTPStatus(String str) {
        try {
            ...
            if (detailedSessionProgress.isEmpty()) {
                ...
                System.out.println(formatEnhancedStatusTable(str, detailedSessionProgress, progressManager));
                progressManager.close();
            }
        } catch (Exception e) {
            ...
            allExerciseStatuses.forEach(exerciseStatus -> {
                if (exerciseStatus.getStatus() != ExerciseManager.ExerciseStatus.Status.NOT_STARTED) {
                    ...
                    if (testProgressData != null) {
                        exerciseStatus.setTestStatus(String.format("%d%% (%d%% / %d%%)", Integer.valueOf(testProgressData.globalPercentage), Integer.valueOf(testProgressData.studentPercentage), Integer.valueOf(testProgressData.professorPercentage)));
                    } else {
                    ...
                    if (parseCsvLineWithBrackets.size() >= 4) {
                        ...
                        if (str.equals(trim2)) {
                            if ("test".equals(trim3) && parseCsvLineWithBrackets.size() >= 9) {
                                try {
                                    TestProgressData testProgressData = new TestProgressData(Integer.parseInt(parseCsvLineWithBrackets.get(5).trim()), Integer.parseInt(parseCsvLineWithBrackets.get(7).trim()), Integer.parseInt(parseCsvLineWithBrackets.get(8).trim()), LocalDateTime.parse(trim, DateTimeFormatter.ISO_LOCAL_DATE_TIME));
                                    TestProgressData testProgressData2 = map.get(trim4);
                                    if (testProgressData2 == null || testProgressData.timestamp.isAfter(testProgressData2.timestamp)) {
                                        map.put(trim4, testProgressData);
                                    }
                                } catch (Exception e) {
                                }
                            } else if ("qcm".equals(trim3) && parseCsvLineWithBrackets.size() >= 6) {
                                try {
                                    int parseInt = Integer.parseInt(parseCsvLineWithBrackets.get(parseCsvLineWithBrackets.size() - 2).trim());
                                    int parseInt2 = Integer.parseInt(parseCsvLineWithBrackets.get(parseCsvLineWithBrackets.size() - 1).trim());
                                    int round = parseInt2 > 0 ? (int) Math.round((parseInt / parseInt2) * 100.0d) : 0;
                                    TestProgressData testProgressData3 = new TestProgressData(round, round, round, LocalDateTime.parse(trim, DateTimeFormatter.ISO_LOCAL_DATE_TIME));
                                    TestProgressData testProgressData4 = map.get(trim4);
                                    if (testProgressData4 == null || testProgressData3.timestamp.isAfter(testProgressData4.timestamp)) {
                                        map.put(trim4, testProgressData3);
                                    }
                                } catch (Exception e2) {
                                }
                            }
    ...
    private String formatEnhancedStatusTable(String str, List<ProgressManager.ExerciseProgress> list, ProgressManager progressManager) {
        StringBuilder sb = new StringBuilder();
        sb.append("Session: ").append(str).append("\n\n");
        sb.append(String.format("%-3s| %-30s | %-6s | %-14s | %-12s | %s\n", "#", "Exercise", "Status", "Score", "C(F) / E / T", "Time"));
        sb.append("---|--------------------------------|--------|----------------|--------------|------\n");
        for (int i = 0; i < list.size(); i++) {
            sb.append(formatEnhancedExerciseLine(i + 1, list.get(i), progressManager, str)).append("\n");
        }
        sb.append("\n");
        int count = (int) list.stream().filter(exerciseProgress -> {
    ...
    private String formatEnhancedExerciseLine(int i, ProgressManager.ExerciseProgress exerciseProgress, ProgressManager progressManager, String str) {
        Object obj;
        Object obj2;
        String str2;
