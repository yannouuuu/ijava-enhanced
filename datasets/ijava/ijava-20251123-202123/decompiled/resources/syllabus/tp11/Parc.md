...
On souhaite représenter un parc pouvant contenir des véhicules. Un parc peut être caractérisé par le nombre véhicules qu'il peut accueillir et une fois ouvert le nombre de véhicules qu'il contient.

::: question
**Quels champs proposeriez-vous pour modéliser un parc de véhicules ?**
- [ ] String[] et int
  > Alors un tableau de chaines de caractères pour les plaques d'immatriculation et un entier pour le nombre de voitures actuellement présentes ? Mais une voiture est plus que sa plaque d'immatriculation ...
- [x] Voiture[] et int
  > Effectivement, bien vu : un parc contient des `Voiture`, mais comme il n'est pas toujours rempli, l'entier permet de savoir où débute la partie non occupée (non initialisée) du parc.
- [ ] Voiture[][] et int
  > Wahou, 2 dimensions ? Quand bien même, cela serait un parc sur plusieurs étages, ce qui nous importe est la capacité totale ... trop complexe et peu judicieux comme choix de structure de données.
- [ ] Voiture[]
  > Alors ceci permet de représenter le parc qui contient les voitures, mais comment déterminer le nombre de voitures présentes dans le parc ? On pourrait s'amuser à rechercher à chaque fois la première valeur `null` dans le tableau, mais cela serait peu efficient !
:::

Définissez le type `Parc` constitué de deux champs :
* `vehicules` qui est un tableau à une dimension de véhicules;
* `nbVehicules` de type entier qui permet de stocker le nombre effectif de véhicules dans le parc (et donc 0 initialement).
