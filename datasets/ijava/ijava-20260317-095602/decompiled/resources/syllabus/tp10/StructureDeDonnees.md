...
* une constante nommée `GAUCHE` et contenant le caractère `<` pour un mouton allant vers la gauche
* une constante nommée `DROITE` et contenant le caractère `<` pour un mouton allant vers la droite
* une constante nommée `ESPACE` et contenant le caractère `_` pour représenter l'espace vide dans la prairie.

Nous allons utiliser l'algorithme principal suivant pour débuter : 
```java 
void algorithm () {
    char[] prairie = initialiser(7);  // Fonction à écrire
