class Wator extends Program {
    ...
    //Paramètres de la simulation
        ...
        final int GESTATION_REQUIN  =  6; // idem pour le requin    final int NOURRITURE_REQUIN =  3; // La nourriture de départ d'un requin
        ...
        void initialiser(Poisson[][] tab, double probaThon, double probaRequin){
            for (int idxL = 0; idxL < length(tab,1); idxL++) {
                for (int idxC = 0; idxC < length(tab,2); idxC++) {
                    tab[idxL][idxC] = newPoisson(probaThon, probaRequin);
                }
            }
        }
        ...
        boolean deplacement(Poisson[][] tab, int lig, int col, Espece espece){
            ...
            for (int i = 0; i < length(directions,1); i++) {
                ...
                // Si c'est un requin et qu'il qui peut manger un thon
        ...
        String toString(Poisson[][] ocean){
            ...
            for (int i = 0; i < length(ocean,1); i++){
                ...
                result += ""; // TODO: retour à la ligne à ajouter
            }
            return result;
        }
